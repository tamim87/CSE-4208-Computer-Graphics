...
public:
	...
	float tox, toy, toz;
	Lcd(float x = 0, float y = 0, float z = 0) {
		tox = x;
		toy = y;
		toz = z;
	}
	...
	Shader local_rotation(Shader ourShader, unsigned int  VAOB, unsigned int VAO4, float angle = 0) {
		glm::mat4 model;
		...
		unsigned int vertex_array[] = { VAOB, VAO4 };
	...
	Shader ret_shader(Shader ourShader, unsigned int  VAOB, unsigned int VAO4 ) {
		glm::mat4 model;
		float rotateAngle_X = 0;
		float rotateAngle_Y = 0;
		float rotateAngle_Z = 0;
