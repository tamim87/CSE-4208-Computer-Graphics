...
int main()
{
    ...
    for (float theta = 0; theta <= 360; theta += 1) {
        ...
        //cylinder.push_back({ x, y, -5.0f }); // normal cylinder
        cylinder.push_back({ x * .3f, y * .3f, -5.0f }); // cylinder with different circle radius
    }
    ...
    //ground texture
    string diffuseMapPathg = "./ground.jpg";
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        tempt = glm::translate(identityMatrix, glm::vec3(0,0,0));
        tmp = tmp * tempt;
        ...
        width = 30.0 + 20.0;
        length = 20.5 + 20.0;
        glm::mat4 modelMatrixForground = glm::mat4(1.0f);
        scaleMatrix = glm::scale(identityMatrix, glm::vec3(width, baseHeight*10, length));
        translateMatrix = glm::translate(identityMatrix, glm::vec3(0, 0, 0));
        //modelMatrixForground = glm::translate(identityMatrix, glm::vec3(-0.45f, -0.4f, -2.8f));
        modelMatrixForground = scaleMatrix * translateMatrix;
        cubeg.drawCubeWithTexture(lightingShaderWithTexture, modelMatrixForground);

        //texture road
        baseHeight = 0.01f;
        width = 30.0f + 20.0f;
        length = 20.5f + 20.0f;
        glm::mat4 modelMatrixForroad = glm::mat4(1.0f);
        scaleMatrix = glm::scale(identityMatrix, glm::vec3(width+1, baseHeight*16, length / 4));
        translateMatrix = glm::translate(identityMatrix, glm::vec3(-1, baseHeight*5, length / 1.6));

        //modelMatrixForground = glm::translate(identityMatrix, glm::vec3(-0.45f, -0.4f, -2.8f));
        modelMatrixForroad = scaleMatrix * translateMatrix;
        cuber.drawCubeWithTexture(lightingShaderWithTexture, modelMatrixForroad);
...
//void shed(unsigned int& cubeVAO, Shader& lightingShader, glm::mat4 alTogether)
//{
//    glm::mat4 model = glm::mat4(1.0f);
//    glm::mat4 translate = glm::mat4(1.0f);
//    glm::mat4 translate2 = glm::mat4(1.0f);
...
void room(unsigned int& cubeVAO, Shader& lightingShader, glm::mat4 alTogether)
{
    ...
    translate = glm::translate(model, glm::vec3(0, 0, 0));
    model = alTogether * scale * translate;
    drawCube(cubeVAO, lightingShader, model, 0.671, 0.408, 0.439);
    ...
    translate = glm::translate(model, glm::vec3(0, 0, 0));
    model = alTogether * translate2 * scale * translate;
    drawCube(cubeVAO, lightingShader, model, 0.035, 0.325, 0.859);
    ...
    translate2 = glm::mat4(1.0f);
    scale = glm::mat4(1.0f);
    translate2 = glm::translate(model, glm::vec3(12 - 1 + 2+tx, ty+baseHeight + 10  - 6 , tz+4 - 1 + 1.8));
